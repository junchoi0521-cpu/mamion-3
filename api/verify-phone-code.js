...
export default async function handler(req, res) {
  ...
  try {
    ...
    const challenge = String(body.phoneVerificationChallenge || '');
    ...
    if (record && Number(record.attempts || 0) >= maxAttempts) {
      return res.status(429).json({ result: 'locked', message: '인증번호 입력 횟수를 초과했습니다. 다시 요청해주세요.' });
    }

    const verifiedByStore = record?.codeHash === hashCode({ phone, code });
    const verifiedByChallenge = verifyCodeChallenge({ challenge, phone, code });
